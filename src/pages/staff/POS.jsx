...
const POS = () => {
  ...
  return (
    <div className="flex min-h-[calc(100vh-5rem)] lg:h-[calc(100vh-5rem)] flex-col bg-[#f5f7fb] lg:overflow-hidden overflow-y-auto">
      <div className="flex flex-1 flex-col lg:flex-row lg:overflow-hidden p-4 lg:p-6 gap-6">
        
        {/* LEFT SECTION (70%) */}
        <div className="flex-[7] flex flex-col min-w-0 gap-6 lg:overflow-hidden">
            ...
            <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-4">
              <div className="flex-1 relative group">
                ...
                <input
                  ...
                  className="w-full pl-14 pr-14 py-4 bg-slate-50 border border-slate-100 rounded-2xl focus:ring-4 focus:ring-primary/10 focus:border-primary focus:bg-white outline-none transition-all font-semibold text-slate-700 placeholder:text-slate-400 text-lg shadow-inner"
                />
                  ...
                  <div className="absolute top-full left-0 right-0 mt-3 z-50 bg-white rounded-3xl shadow-2xl border border-slate-100 overflow-hidden max-h-[440px] overflow-y-auto custom-scrollbar ring-8 ring-black/5 animate-in fade-in slide-in-from-top-2 duration-300">
                    {searchResults.length === 0 ? (
                      <div className="p-10 text-center text-slate-400">
                        ...
                        <p className="text-xs font-normal mt-1 opacity-60">Thử tìm kiếm với từ khóa khác</p>
                      </div>
                    ) : (
                      <div className="p-2">
                        {searchResults.map((product) => (
                          ...
                          >
                            ...
                            <div className="flex-1 min-w-0">
                              <p className="font-semibold text-slate-800 truncate group-hover/item:text-primary transition-colors">{product.productName}</p>
                              <div className="flex items-center gap-2 mt-0.5">
                                <span className="px-1.5 py-0.5 rounded-md bg-slate-100 text-slate-500 text-[10px] font-semibold uppercase">#{product.productCode}</span>
                                <span className={`px-1.5 py-0.5 rounded-md text-[10px] font-semibold uppercase ${product.stock_quantity > 5 ? 'bg-emerald-50 text-emerald-600' : 'bg-red-50 text-red-600'}`}>
                                  Tồn: {product.stock_quantity}
                                </span>
                              </div>
                            </div>
                            <div className="text-right">
                              <p className="font-bold text-primary text-lg leading-none">{Number(product.salePrice).toLocaleString()} đ</p>
                              <p className="text-[10px] font-normal text-slate-300 mt-1 uppercase">Click để thêm</p>
                            </div>
                          </button>
                        ))}
                      </div>
              ...
              <div className="flex items-center gap-3 w-full sm:w-auto justify-end sm:justify-start">
                <button className="flex-1 sm:flex-initial flex items-center justify-center gap-2 px-5 py-4 bg-white border border-slate-200 rounded-2xl font-semibold text-slate-600 hover:bg-slate-50 hover:border-slate-300 transition-all shadow-sm">
          ...
          <div className="bg-white rounded-[24px] shadow-sm border border-slate-100 flex-1 flex flex-col overflow-hidden relative">
            <div className="flex-1 overflow-y-auto custom-scrollbar">
              ...
              <table className="hidden lg:table w-full text-left border-collapse">
                <thead className="sticky top-0 bg-white/95 backdrop-blur-md z-10 border-b border-slate-50">
                  <tr className="text-[11px] font-semibold text-slate-400 uppercase tracking-[0.1em]">
                    ...
                    cart.map((item, index) => (
                      <tr key={item.id} className="group hover:bg-slate-50/50 transition-colors">
                        <td className="pl-8 py-4 text-center">
                          <span className="text-xs font-semibold text-slate-300 group-hover:text-primary">{index + 1}</span>
                        </td>
                        <td className="px-4 py-4">
                          <div className="flex items-center gap-4">
                            ...
                            <div className="min-w-0">
                              <p className="font-semibold text-slate-800 text-sm mb-0.5 truncate">{item.productName}</p>
                              <p className="text-[10px] font-medium text-slate-400 tracking-wider uppercase flex items-center gap-2">
                        ...
                        <td className="px-4 py-4 text-right">
                          <span className="font-semibold text-slate-700 text-sm">{Number(item.salePrice).toLocaleString()} đ</span>
                        </td>
                        <td className="px-4 py-4">
                          <div className="flex items-center justify-center">
                            <div className="flex items-center bg-white border border-slate-200 rounded-xl p-0.5 shadow-sm transition-all focus-within:ring-2 focus-within:ring-primary/20">
                              ...
                              <input 
                                ...
                                className="w-10 text-center font-bold text-slate-800 bg-transparent outline-none text-sm"
                              />
                              <button 
                                onClick={() => updateCartQuantity(item.id, item.quantity + 1, item.stock_quantity)}
                                className="w-8 h-8 flex items-center justify-center text-slate-400 hover:text-primary transition-colors"
                        ...
                        <td className="px-4 py-4 text-right">
                          <span className="font-bold text-primary text-sm">{(item.salePrice * item.quantity).toLocaleString()} đ</span>
                        </td>
              ...
              <div className="lg:hidden divide-y divide-slate-100 p-4 space-y-4">
                {cart.length === 0 ? (
                  <div className="flex flex-col items-center justify-center text-center py-20 opacity-40">
                    <div className="w-20 h-20 bg-slate-50 rounded-[2rem] flex items-center justify-center mb-4 border-2 border-dashed border-slate-200">
                       <ScanLine className="w-10 h-10 text-slate-300" />
                    </div>
                    <p className="text-lg font-black text-slate-500">Giỏ hàng đang trống</p>
                    <p className="text-xs font-bold text-slate-400 mt-1">Quét mã vạch để bắt đầu bán hàng</p>
                  </div>
                ) : (
                  cart.map((item, index) => (
                    <div key={item.id} className="flex flex-col gap-3 pb-4 last:pb-0">
                      <div className="flex items-start gap-3">
                        ...
                        <div className="flex-1 min-w-0">
                          ...
                          <p className="text-[10px] font-medium text-slate-400 uppercase mt-0.5 tracking-wider flex items-center gap-1">
                            <Barcode className="w-3 h-3" /> {item.productCode}
                          </p>
                        </div>
                        {/* Delete Button */}
                      ...
                      <div className="flex items-center justify-between mt-1">
                        ...
                        <div className="flex items-center bg-slate-50 border border-slate-100 rounded-xl p-0.5 shadow-inner">
                          <button 
                            onClick={() => updateCartQuantity(item.id, item.quantity - 1, item.stock_quantity)}
                          ...
                          <input 
                            ...
                            value={item.quantity}
                            onChange={(e) => updateCartQuantity(item.id, Number(e.target.value), item.stock_quantity)}
                            className="w-8 text-center font-bold text-slate-800 bg-transparent outline-none text-xs"
                          />
                          <button 
                            onClick={() => updateCartQuantity(item.id, item.quantity + 1, item.stock_quantity)}
                            className="w-7 h-7 flex items-center justify-center text-slate-400 hover:text-primary transition-colors"
                        ...
                        {/* Price Subtotal */}
                        <div className="text-right">
                          <span className="text-xs text-slate-400 block">{Number(item.salePrice).toLocaleString()} đ/sp</span>
                          <span className="font-bold text-primary text-sm">{(item.salePrice * item.quantity).toLocaleString()} đ</span>
                        </div>
                      </div>
                    </div>
            ...
            <div className="bg-white border-t border-slate-100 p-6 flex flex-col sm:flex-row items-center justify-between gap-6 shrink-0 shadow-[0_-10px_20px_-10px_rgba(0,0,0,0.03)]">
               <div className="grid grid-cols-3 gap-4 w-full sm:flex sm:items-center sm:gap-8 sm:w-auto">
                  <div className="flex flex-col">
                     <span className="text-[10px] sm:text-[11px] font-semibold text-slate-400 uppercase tracking-widest mb-1">Tổng số lượng</span>
                     <div className="flex items-center gap-1.5 sm:gap-2">
                        <Package className="w-4 h-4 text-slate-300 shrink-0" />
                        <span className="font-bold text-slate-800 text-base sm:text-xl">{cart.reduce((a,c) => a+c.quantity, 0)} <span className="text-[10px] sm:text-xs font-normal text-slate-400">sản phẩm</span></span>
                     </div>
                  </div>
                  <div className="hidden sm:block h-10 w-[2px] bg-slate-50"></div>
                  <div className="flex flex-col">
                     <span className="text-[10px] sm:text-[11px] font-semibold text-slate-400 uppercase tracking-widest mb-1">Tạm tính</span>
                     <div className="flex items-center gap-1.5 sm:gap-2">
                        <ShoppingCart className="w-4 h-4 text-slate-300 shrink-0" />
                        <span className="font-bold text-slate-800 text-base sm:text-xl">{totalAmount.toLocaleString()} đ</span>
                     </div>
                  </div>
                  <div className="hidden sm:block h-10 w-[2px] bg-slate-50"></div>
                  <div className="flex flex-col">
                     <span className="text-[10px] sm:text-[11px] font-semibold text-slate-400 uppercase tracking-widest mb-1">Giảm giá</span>
                     <div className="flex items-center gap-1.5 sm:gap-2">
                        <span className="font-bold text-slate-800 text-base sm:text-xl">0 đ</span>
                        <button className="p-1 hover:bg-slate-50 rounded text-slate-400"><Plus className="w-4 h-4" /></button>
                     </div>
                  </div>
               </div>
               <div className="flex items-center gap-3 w-full sm:w-auto justify-between sm:justify-end border-t border-slate-50 pt-4 sm:pt-0 sm:border-t-0">
                  <div className="text-right w-full sm:w-auto flex items-center justify-between sm:block gap-4">
                    <span className="text-[11px] font-semibold text-slate-400 uppercase tracking-widest block mb-0.5">Tổng cộng</span>
                    <span className="text-2xl sm:text-4xl font-bold text-primary leading-none tracking-tighter">{totalAmount.toLocaleString()} <span className="text-xs sm:text-sm font-normal opacity-50">VND</span></span>
                  </div>
               </div>
            </div>
          </div>
        ...
        {/* RIGHT SECTION (30%) */}
        <div className="flex-[3] flex flex-col w-full lg:min-w-[360px] gap-6 lg:overflow-y-auto custom-scrollbar lg:h-full">
          ...
          <div className="bg-white rounded-[24px] shadow-sm border border-slate-100 flex flex-col overflow-hidden transition-all hover:shadow-md">
            ...
            <div className="p-6 space-y-4">
              <div className="flex justify-between items-center">
                <span className="text-slate-400 font-medium text-sm">Số lượng sản phẩm</span>
                <span className="font-semibold text-slate-700">{cart.reduce((a,c) => a+c.quantity, 0)}</span>
              </div>
              <div className="flex justify-between items-center">
                <span className="text-slate-400 font-medium text-sm">Tạm tính</span>
                <span className="font-semibold text-slate-700">{totalAmount.toLocaleString()} đ</span>
              </div>
              <div className="flex justify-between items-center">
                <span className="text-slate-400 font-medium text-sm">Thuế VAT (0%)</span>
                <span className="font-semibold text-slate-700">0 đ</span>
              </div>
              <div className="flex justify-between items-center">
                <span className="text-slate-400 font-medium text-sm">Giảm giá</span>
                <span className="font-semibold text-emerald-500">0 đ</span>
              </div>
              
              <div className="pt-6 mt-2 border-t-2 border-slate-100 border-dashed">
                <div className="flex justify-between items-end">
                  <span className="text-sm font-semibold text-slate-400 uppercase tracking-widest mb-1">Tổng cộng</span>
                  <span className="text-4xl font-bold text-primary leading-none tracking-tighter">
                    {totalAmount.toLocaleString()} <span className="text-xs font-normal opacity-50 uppercase">đ</span>
                  </span>
                </div>
              </div>
            </div>
            ...
            <div className="grid grid-cols-3 gap-3">
              {[
                ...
                >
                  ...
                  <span className="text-[10px] font-semibold uppercase tracking-widest text-center leading-tight">{method.label}</span>
                </button>
              ))}
            </div>
            ...
            {paymentMethod === 'cash' && (
              <div className="mt-2 space-y-5 animate-in fade-in slide-in-from-top-4 duration-300">
                <div className="relative group">
                  <label className="text-[10px] font-semibold text-slate-400 uppercase tracking-widest absolute -top-2 left-4 bg-white px-2 z-10">
                    Tiền khách đưa
                  </label>
                  <input 
                    type="number"
                    ...
                    className="w-full pl-6 pr-14 py-5 bg-slate-50 border border-slate-100 rounded-2xl font-bold text-2xl text-slate-800 focus:ring-4 focus:ring-primary/10 focus:border-primary focus:bg-white outline-none transition-all placeholder:text-slate-300"
                    placeholder="0.000"
                  />
                ...
                <div className="p-5 bg-emerald-50/50 rounded-[24px] border border-emerald-100 flex flex-col gap-1 relative overflow-hidden group">
                  ...
                  <span className="text-[10px] font-semibold text-emerald-600 uppercase tracking-widest relative z-10">Tiền thừa trả lại</span>
                  <span className={`text-4xl font-bold relative z-10 tracking-tighter ${changeAmount > 0 ? 'text-emerald-700' : 'text-emerald-200'}`}>
                    {changeAmount.toLocaleString()} <span className="text-sm font-normal opacity-60">đ</span>
                  </span>
                </div>
              </div>
            )}
          ...
          <div className="mt-auto space-y-4">
            <button
              ...
              className="w-full py-6 bg-gradient-to-r from-primary to-primary-dark hover:from-primary-dark hover:to-primary-dark text-white rounded-[24px] font-bold text-xl shadow-xl shadow-primary/25 hover:shadow-primary/40 transition-all active:scale-95 disabled:opacity-50 disabled:grayscale disabled:shadow-none flex items-center justify-center gap-4 group overflow-hidden relative"
            >
            ...
            <div className="grid grid-cols-3 gap-3">
              <button className="flex flex-col items-center justify-center gap-2 p-4 bg-white border border-slate-100 rounded-[20px] shadow-sm hover:bg-slate-50 transition-all active:scale-95 group">
                ...
                <span className="text-[10px] font-semibold text-slate-400 uppercase tracking-widest">Lưu tạm</span>
              </button>
              <button onClick={() => window.print()} className="flex flex-col items-center justify-center gap-2 p-4 bg-white border border-slate-100 rounded-[20px] shadow-sm hover:bg-slate-50 transition-all active:scale-95 group">
                ...
                <span className="text-[10px] font-semibold text-slate-400 uppercase tracking-widest">In hóa đơn</span>
              </button>
              <button onClick={() => updateCurrentTab({ cart: [], receivedAmount: 0 })} className="flex flex-col items-center justify-center gap-2 p-4 bg-white border border-slate-100 rounded-[20px] shadow-sm hover:bg-red-50 hover:border-red-100 transition-all active:scale-95 group">
                ...
                <span className="text-[10px] font-semibold text-slate-400 uppercase tracking-widest">Xóa tất cả</span>
              </button>
            </div>
          </div>
        </div>
          ...
          <div className="bg-white w-full max-w-2xl rounded-[40px] shadow-2xl overflow-hidden animate-in zoom-in-95 duration-300 border border-slate-100 flex flex-col max-h-[90vh] print:max-h-none print:shadow-none print:border-none print:rounded-none print:w-auto">
            ...
            <div className="bg-white px-8 py-5 border-b border-slate-50 flex items-center justify-between print:hidden">
              <div className="flex gap-2 p-1.5 bg-slate-50 rounded-2xl">
                <button 
                  ...
                  className={`px-6 py-2.5 rounded-xl text-[10px] font-bold uppercase tracking-widest transition-all ${receiptType === 'pos' ? 'bg-white text-primary shadow-sm ring-1 ring-slate-100' : 'text-slate-400 hover:text-slate-600'}`}
                >
                  Bill K80
                </button>
                <button 
                  onClick={() => setReceiptType("a4")}
                  className={`px-6 py-2.5 rounded-xl text-[10px] font-bold uppercase tracking-widest transition-all ${receiptType === 'a4' ? 'bg-white text-primary shadow-sm ring-1 ring-slate-100' : 'text-slate-400 hover:text-slate-600'}`}
                >
                  Hóa đơn A4
                </button>
              </div>
            ...
            <div className="p-8 bg-white border-t border-slate-50 flex gap-4 print:hidden">
              <button 
                ...
                className="flex-[2] py-5 bg-slate-900 hover:bg-slate-950 text-white font-bold rounded-2xl transition-all shadow-xl shadow-slate-900/20 flex items-center justify-center gap-3 text-xs uppercase tracking-widest active:scale-95"
              >
              ...
              <button 
                ...
                className="flex-1 py-5 bg-emerald-600 hover:bg-emerald-700 text-white font-bold rounded-2xl transition-all shadow-xl shadow-emerald-600/20 flex items-center justify-center gap-3 text-xs uppercase tracking-widest active:scale-95"
              >
